...
class WordDictionary {
     ...
     * @return {boolean}
     */
    private searchWordDFS(word: string, startIdx: number, root: TrieNode | null): boolean {
        let cur = root

        for (let i = startIdx; i < word.length; i++) {
            if (word[i] === '.') {
                for (const child of cur.children) {
                    if (child !== null && this.searchWordDFS(word, i + 1, child)) {
                        return true;
                    }
                }
                return false
            } else {
                let charIdx = word[i].charCodeAt(0) - 'a'.charCodeAt(0)
                if (cur.children[charIdx] === null) {
                    return false
                }
                cur = cur.children[charIdx]
    ...
    search(word: string): boolean {
        return this.searchWordDFS(word, 0, this.root)
    }
}
